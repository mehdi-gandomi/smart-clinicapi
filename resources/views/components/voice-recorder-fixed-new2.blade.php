...
<div
    x-data="{
        ...
        async startRecording() {
            try {
                ...
                this.mediaRecorder.onstop = () => {
                    ...
                    formData.append('{{ $statePath }}_duration', this.duration);

                    // Dispatch a custom event with the FormData
                    this.$dispatch('voice-recorder-updated', { formData });
        ...
        deleteRecording() {
            ...
            // Create a new FormData object with empty values
            const formData = new FormData();
            formData.append('{{ $name }}', '');
            formData.append('{{ $statePath }}_duration', '');

            // Dispatch a custom event with the empty FormData
            this.$dispatch('voice-recorder-updated', { formData });
    ...
    class="space-y-4"
    @voice-recorder-updated.window="
        $event.detail.formData.forEach((value, key) => {
